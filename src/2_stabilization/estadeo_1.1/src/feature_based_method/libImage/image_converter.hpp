...
void rgb2gray(float *I, ImageOut * imaOut, int width, int height) {

  (*imaOut) = ImageOut(width, height);
  for(size_t j = 0; j < height; ++j)
    for(size_t i = 0; i < width; ++i)  {
      (*imaOut)(j,i)=(unsigned char) (0.2127*I[(j*width+i)*3] + 
            0.7152*I[(j*width+i)*3+1] + 0.0722*I[(j*width+i)*3+2]);
    }
}
