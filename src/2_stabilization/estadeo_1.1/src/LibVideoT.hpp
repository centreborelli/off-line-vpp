...
//		                (unsigned long int) getpid());
	...
	,   float &o_rmse
		...
		{
			double mse_prev = mse;
			double term_k = i_vid1(k) - i_vid2(k);
	...
	/**
	 * @brief Compute RMSE between i_vid1 and i_vid2 like they did in SPTWO paper
	 *
	 * @param i_vid1 : video 1;
	 * @param i_vid2 : video 2;
	 *
	...
	,   Video<T> const& i_vid2
		...
		// using doubles
		double rmse = 0.f;
		for(unsigned c = 0; c < i_vid1.sz.channels; ++c)
		{
			double sum = 0.f;
			for (unsigned i = 0; i < i_vid1.sz.width; ++i)
			for (unsigned j = 0; j < i_vid1.sz.height; ++j)
			for (unsigned k = 0; k < i_vid1.sz.frames; ++k)
				sum += ((double)i_vid1(i,j,k,c) - (double)i_vid2(i,j,k,c)) *
					((double)i_vid1(i,j,k,c) - (double)i_vid2(i,j,k,c));

			rmse += sqrtf(sum / (double) (i_vid1.sz.width * i_vid1.sz.height * i_vid1.sz.frames));
		}
		rmse /= (double) i_vid1.sz.channels;
