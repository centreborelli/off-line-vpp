...
void compute_patch_from_temporal_window(image_double_t patch, image_double_t noisemap,
                                        struct fba_process_inputs* pi, struct fba_process_buffers* pb)
{
    int W = patch.w;
    int d = patch.d;
    ...
    for (int m = 0; m < pi->n; m++)
    {
        ...
        weights[m] = new_image_double(W, W, 1);
        for (int j = 0; j < W*W; j++)
        {
    ...
    for (int j = 0; j < W*W; j++)
    {
        double sum = 0.;
        for (int m = 0; m < pi->n; m++)
            sum += weights[m].data[j];
        for (int m = 0; m < pi->n; m++)
            noisefactor += pow(weights[m].data[j] / sum, 2.);
    }
    noisefactor /= W * W;
    for (int j = 0; j < W*W; j++)
    {
        double v = pi->window.data[j];
        noisemap.data[j] = noisefactor * v;
    }
    for (int m = 0; m < pi->n; m++)
        free(weights[m].data);
}

void fba(image_float_t out, image_float_t noisefactor, image_float_t* inputs, int W, float p, int n)
{
...
#pragma omp critical (fftw)
    ...
    image_double_t noisefactor_accum = new_image_double(pw, ph, 1);
    ...
    set_value(noisefactor_accum, 0.);

    image_double_t patch = new_image_double(W, W, d);
    image_double_t noisefactor_patch = new_image_double(W, W, 1);
    ...
    for (int y = 0; y < h - W/2; y += W / 2)
    {
        for (int x = 0; x < w - W/2; x += W / 2)
        {
            ...
            compute_patch_from_temporal_window(patch, noisefactor_patch, &process_inputs, &process_buffers);

            // accumulate the result and the window
            accumulate(output_padded, patch, x, y);
            accumulate(windowing_accum, window, x, y);
            accumulate(noisefactor_accum, noisefactor_patch, x, y);
        }
    }
    ...
    for (int j = 0; j < pw*ph; j++)
    {
        ...
        noisefactor_accum.data[j] /= v;
    }

    // extract the interesting part of the image
    crop(out, output_padded);
    crop(noisefactor, noisefactor_accum);

    free(patch.data);
    free(noisefactor_patch.data);
